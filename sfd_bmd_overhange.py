...
class MovingTruckBeam(Scene):
    def construct(self):
        ...
        # ValueTracker for linear pedestrian movement across the structure
        truck_x = ValueTracker(-0.5)

        # Configured for a single, heavy moving point load
        axles = [
        ...
        def draw_peak_deflection_label():
            ...
            if abs(max_def_mm) > 0.1:
                ...
                lbl = Text(f"{abs(max_def_mm):.1f} mm", color="#34D399", weight="BOLD").scale(0.20)
        ...
        # Support Icons
        hinge = Triangle(color="#64748B", fill_color="#334155", fill_opacity=1).scale(0.14).move_to([sx(0), Y_BEAM - 0.14, 0])
        roller = Circle(color="#64748B", fill_color="#334155", fill_opacity=1).scale(0.08).move_to([sx(L_s), Y_BEAM - 0.08, 0])
        ...
        def draw_pedestrian_load():
            ...
            for P, off in axles:
                ...
                if 0 <= axle_pos <= L:
                    ...
                    # Minimalist Architectural Silhouette Blueprint 
                    head = Circle(radius=0.07, color="#38BDF8", fill_color="#38BDF8", fill_opacity=0.4, stroke_width=1.5).move_to([pos_x, base_y + 0.62, 0])
                    torso = Line([pos_x, base_y + 0.55, 0], [pos_x, base_y + 0.32, 0], color="#38BDF8", stroke_width=3)
                    left_leg = Line([pos_x, base_y + 0.32, 0], [pos_x - 0.05, base_y + 0.12, 0], color="#38BDF8", stroke_width=2.5)
                    right_leg = Line([pos_x, base_y + 0.32, 0], [pos_x + 0.05, base_y + 0.12, 0], color="#38BDF8", stroke_width=2.5)
                    left_arm = Line([pos_x, base_y + 0.50, 0], [pos_x - 0.06, base_y + 0.35, 0], color="#38BDF8", stroke_width=2)
                    right_arm = Line([pos_x, base_y + 0.50, 0], [pos_x + 0.06, base_y + 0.35, 0], color="#38BDF8", stroke_width=2)
                    
                    person = VGroup(head, torso, left_leg, right_leg, left_arm, right_arm)
                    g.add(person)

                    # Concentrated Vector Force Arrow
                    arrow_length = P * 0.01 
                    arrow = Arrow(
                    ...
                    txt = Text(f"{P} kN", color="#EF4444", weight="BOLD").scale(0.18)
        ...
        def render_reactions():
            RA, RB = reactions()
            t1 = Text(f"RA = {RA:.1f} kN", color="#F59E0B", weight="BOLD").scale(0.22).move_to([sx(0), Y_BEAM - 0.45, 0])
            t2 = Text(f"RB = {RB:.1f} kN", color="#F59E0B", weight="BOLD").scale(0.22).move_to([sx(L_s), Y_BEAM - 0.45, 0])
            return VGroup(t1, t2)
        ...
        infl_poly = VMobject(fill_color="#F59E0B", fill_opacity=0.1, stroke_color="#F59E0B", stroke_width=1.5)
        ...
        def draw_influence_marker():
            ...
            val_lbl = Text(f"η = {1-(leading_x/L_s):.2f}", color="#F59E0B", weight="BOLD").scale(0.20)
        ...
        def create_sfd_diagram():
            ...
            if v_eval:
                ...
                max_v = v_eval[p_idx]
                if abs(max_v) > 2.0:
                    dot = Dot([sx(x_eval_pos[p_idx]), Y_SFD + max_v * SHEAR_SCALE, 0], radius=0.04, color="#FFFFFF")
                    lbl = Text(f"{max_v:.1f} kN", color="#F8FAFC", weight="BOLD").scale(0.20)
        ...
        def create_bmd_diagram():
            ...
            if vals:
                ...
                max_m = vals[p_idx]
                if abs(max_m) > 2.0:
                    dot = Dot([sx(nodes[p_idx]), Y_BMD + max_m * MOMENT_SCALE, 0], radius=0.04, color="#FFFFFF")
                    lbl = Text(f"{max_m:.1f} kNm", color="#F8FAFC", weight="BOLD").scale(0.20)
        ...
        self.wait(0.5)
        self.play(
            truck_x.animate.set_value(L + 0.5),
            run_time=12,
            rate_func=linear
        )
        self.wait(1.0)